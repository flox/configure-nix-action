...
if [ -f /tmp/drv-paths ]; then
  ...
  awk 'NR==FNR{a[$0]=1;next}!a[$0]' /tmp/dependency-paths /tmp/dependency-paths-outputs | xargs nix copy --extra-experimental-features nix-command --to "$CONFIGURE_NIX_SUBSTITUTER" ||:;
fi
